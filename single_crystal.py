    ...
    def omega(dTeta):  # скан одной щелью относительно второй
        ...
        # 1-------------------------------------------------------------------------------------------------------------------
            ...
            while itta <= itta_2:
                ...
                z_promegutochn_lin = []
                while teta <= teta_2:
                    P = g_lambd(itta, wavelength_1, wavelength_2)*gauss(sigma, 0, math.degrees(
                            teta)*3600)*sample_curve(dTeta, teta, itta, X0_1, Xh_1, bragg_1, fi_1)
    ...
    def theta(dTeta):  # скан одной щелью относительно второй
        ...
        # 1-------------------------------------------------------------------------------------------------------------------
            ...
            while itta <= itta_2:
                ...
                while teta <= teta_2:
                    P = g_lambd(itta, wavelength_1, wavelength_2)*gauss(sigma, 0, math.degrees(
                            teta)*3600)*monohromator_curve(teta, itta, X0_1, Xh_1, bragg_1, fi_1)
                    x_promegutochn.append(itta*wavelength_1*1e10)
                    y_promegutochn.append(math.degrees(teta)*3600)
                    z_promegutochn.append(math.log10(P))
                    z_promegutochn_lin.append(P)
